...
# This setup is generally ok for Virtual Machines
	if [ "$1" = "vm" ]; then
	vm_dirty_bg_ratio=5
	vm_dirty_ratio=10
...
# This setup is generally for both (high traffic burst on slow disks)
	vm_dirty_bg_ratio=5
	vm_dirty_ratio=80
	fi
...
echo "Activating new settings from sysctl.conf"
